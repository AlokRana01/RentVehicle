...
def fetchregister(request):
    if request.method == "POST":
        ...
        hashed_password = make_password(u_password) if u_password else None
...
def login(request):
    if request.method == "POST":
        ...
        user = None
        try:
            candidate = registermodel.objects.get(u_email=Email1)
            # Verify hashed password or legacy plaintext
            if candidate.u_password:
                if check_password(Password1, candidate.u_password) or candidate.u_password == Password1:
                    user = candidate
                    # Upgrade legacy plaintext password to secure hash automatically
                    if candidate.u_password == Password1:
                        candidate.u_password = make_password(Password1)
                        candidate.save()
        except registermodel.DoesNotExist:
            user = None
...
def book_car_view(request):
    if request.method == "POST":
        ...
        redirect_target = f"/booking/{vehicle_id}" if vehicle_id else "/car_list_page"
        try:
...
def payment_success(request):
    if request.method == "POST":
        try:
            ...
            # Server-side verification using Razorpay client utility
